            ...
            <table class="table table-striped">
                ...
                @foreach ($products as $product)
                    <tr>
                        ...
                        <td>
                            <div class="cart-info">
                            <img src="/image/{{ $product->image }}" width="200px" class = "img-fluid">
                            </div>
                        </td>
                        <td>{{ $product->price }}</td>
                        <form action="{{ route('products.destroy',$product->id) }}" method="POST">
                                    <td><a class="btn btn-primary" style="font-size: 12px;" href="{{ route('products.edit',$product->id) }}">Edit</a></td>
                                    @csrf
                                    @method('DELETE')
                                    <td><button type="submit" style="font-size: 12px;" class="btn btn-danger">Delete</button></td>
                        </form>
                    </tr>
                    @endforeach
                </tbody>
